...
void *memory_alloc(unsigned long int bytes)
{
  ...
  void* pointer = brk_inicial;
  void* biggest_pointer = 0;
  unsigned long int biggest_size = 0;
  unsigned long int size = 0;

  while (pointer < max_pointer_val)
  {
    size = *((unsigned long int*) (pointer + 8));
    if (*((unsigned long int*) pointer) == 0 && size >= bytes)
    {
